...
export default function PropaneCustomizer() {
  ...
  const [propaneColor, setPropaneColor] = useState('#ffffff')
  ...
  renderer.setClearColor(0x000000, 0)
    ...
    loader.load('/models/propane.gltf', (gltf) => {
      const propaneModel = gltf.scene
      meshRef.current = propaneModel
      
      const box = new THREE.Box3().setFromObject(propaneModel)
      const size = box.getSize(new THREE.Vector3())
      const maxDim = Math.max(size.x, size.y, size.z)
      const scale = 2 / maxDim
      propaneModel.scale.multiplyScalar(scale)
      
      box.setFromObject(propaneModel)
      const center = box.getCenter(new THREE.Vector3())
      propaneModel.position.set(-center.x, -center.y, -center.z)
      
      scene.add(propaneModel)
  ...
  const updateTexture = (overlaysToRender = overlays) => {
    ...
    // Convert grey propane colors to white
    let fillColor = propaneColor
    // Parse hex color to RGB
    const hex = propaneColor.replace('#', '')
    ...
    // Clear canvas with cup color background
  ...
  useEffect(() => {
    const handleLoadDesign = (e) => {
      ...
      if (design.type !== 'propane') return
      
      // Load the design data
      setPropaneColor(design.colorHex || '#ffffff')
    ...
    if (framesJson) {
      try {
        ...
        if (frames && frames.length > 0) {
          ...
          console.log('Auto-loading generated frame for propane:', frameUrl)
          ...
          img.onload = () => {
            ...
            console.log('Added generated frame as overlay for propane:', newOverlay)
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      type: 'propane',
      price: 35.00,
      color: colorNames[propaneColor] || 'Custom',
      colorHex: propaneColor,
    ...
    <main style={{ 
      ...
      <div style={{ 
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '12px', padding: '12px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: '#666', fontSize: '0.9rem' }}>Custom Propane Tank · {propaneColor}</div>
                </div>
          ...
          <div style={{
            ...
            <div 
              ...
              style={{
                ...
                aspectRatio: '1',
                cursor: 'crosshair',
                background: `url(/uv-maps/propane-uv.png)`,
            ...
            <div style={{
              ...
              Click and drag to position on propane tank
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          {itemSelectedMode ? 'Item selected - Click away to rotate propane tank' : '← Click and drag to rotate →'}
        </div>
      </div>
      ...
      <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              <button
                ...
                onClick={() => setPropaneColor(colorOption.color)}
                style={{
                  width: '100%',
                  aspectRatio: '1',
                  background: colorOption.color,
                  border: `3px solid ${propaneColor === colorOption.color ? '#000' : '#ddd'}`,
                  borderRadius: '8px',
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                  boxShadow: propaneColor === colorOption.color ? '0 0 0 2px #4db8ff' : 'none',
                }}
        ...
        >
          {overlays.length === 0 ? 'Add designs to continue' : 'Add to Cart - $35.00'}
        </button>
      </div>
